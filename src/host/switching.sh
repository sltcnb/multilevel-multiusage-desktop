...
EOF
  ...
  # this module surfaces that verdict on the bar. It must NEVER crash and never
  # write to stderr (polybar would paint the error into the bar): a missing or
  # unparsable file is simply UNKNOWN. ISOLATION_STATUS_FILE overrides the path
  # for tests; production uses the default. The lock glyph is the one already
  # used by vpn.sh — the bar's font subset has no dedicated shield in use.
...
module-margin = 2
modules-left  = env
modules-right = isolation cpu memory net vpn battery date
...
Next (operator): create + isolate the VMs -> cd /opt/appliance && ./setup-machine.sh
EOF
